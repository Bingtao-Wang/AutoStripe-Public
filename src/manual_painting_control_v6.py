#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""AutoStripe V6 - V5 + RViz 3-Panel Display

Based on V5, adds:
- ROS image publishing for 3 RViz panels (optional, via try/except)
  Panel 1: RGB front camera + edge overlay (1248x384)
  Panel 2: SNE normal map (LUNA) / depth heatmap (VLLiNet/GT) (1248x384)
  Panel 3: Overhead bird's eye view with paint trail (downscaled 900x800)
- _render_front_view() returns BGR composite for Panel 1
- Panel 2 builder: SNE normal -> RGB vis or depth COLORMAP_MAGMA

ROS is optional: works identically to V5 without ROS installed.

Keyboard Controls: same as V5
Usage:
  1. Start CARLA: ./CarlaUE4.sh
  2. Run: python manual_painting_control_v6.py
  3. (Optional) RViz: roslaunch autostripe autostripe_v6.launch
"""
...
try:
    import rospy
    from ros_interface.rviz_publisher_v6 import RvizPublisherV6, MapView
...
class AutoPaintStateMachine:
    ...
    GRACE_LIMIT = 300         # PAINTING grace frames before downgrade
    STABILIZED_GRACE = 100    # STABILIZED grace frames before downgrade
...
class ManualPaintingControl:
    ...
    def __init__(self, vehicle):
        ...
        self.paint_trail = []
        self.paint_trail_dists = []  # edge_dist_r for each trail point
    ...
    def paint_line(self, world, nozzle_loc, edge_dist=None):
        ...
        if self.dash_mode and self.last_nozzle_loc is not None:
            ...
            if self._dash_accum >= threshold:
                ...
                if not self._dash_painting:
                    ...
                    self.paint_trail_dists.append(None)
            ...
            if self._dash_painting:
                ...
                self.paint_trail_dists.append(edge_dist)
...
def main():
    ...
    print("=" * 60)
    print("  AutoStripe V6 - Triple Perception + RViz 3-Panel Display")
    ...
    pygame.display.set_caption("AutoStripe V6 - Front View + Control")
    ...
    try:
        ...
        # 4c. V6: ROS init + RViz publisher (optional)
        rviz_pub = None
        rviz_3d = None
        if ROS_AVAILABLE:
            try:
                ...
                if not rosgraph.is_master_online():
                    raise RuntimeError("ROS master not running")
                rospy.init_node('autostripe_v6', disable_signals=True)
                rviz_pub = RvizPublisherV6()
                rviz_3d = RvizPublisher()
                print("  [V6] ROS node initialized, RViz publishing enabled (image + 3D)")
            except Exception as e:
                print(f"  [V6] ROS not available ({e}), running without RViz")
                rviz_pub = None
                rviz_3d = None

        # 4d. V6: 2D map view (works with or without ROS)
        map_view = None
        try:
            map_view = MapView(scene['map'])
            print("  [V6] MapView initialized")
        except Exception as e:
            print(f"  [V6] MapView init failed: {e}")
        ...
        while True:
            ...
            # V5.2: nozzle-centric two-stage offset (no PD, no set_dynamic_offset)
            ...
            # --- Visualize: 3D debug dots disabled in V6 (pollutes depth camera) ---
            ...
            # (3D driving path + poly curve dots also disabled in V6)
            ...
            if nozzle_edge_pt is not None:
                ...
                # V6: green debug line disabled (pollutes depth/semantic cameras)
            ...
            # V5.2.1: use edge_dist_r directly with NED median filter
            dist_for_sm = edge_dist_r if edge_dist_r < 900 else 3.0
            ned_history.append(dist_for_sm)
            if len(ned_history) > NED_SMOOTH_WINDOW:
                ned_history.pop(0)
            dist_for_sm = float(np.median(ned_history))
            ...
            paint_ctrl.paint_line(world, nozzle_loc, edge_dist=edge_dist_r)
            ...
            # --- Render front view in pygame (V6: returns BGR for RViz) ---
            front_bgr = _render_front_view(pg_screen, rgb_front, road_mask, scene,
            ...
            # --- V6: Build Panel 2 (perception detail) ---
            panel2_bgr = None
            if rviz_pub is not None:
                panel2_bgr = _build_perception_detail(
                    perception, depth_data, depth_color)
            ...
            if map_view is not None:
                try:
                    veh_tf = vehicle.get_transform()
                    veh_xy = (veh_tf.location.x, veh_tf.location.y)
                    trail = paint_ctrl.paint_trail if hasattr(paint_ctrl, 'paint_trail') else None
                    trail_dists = paint_ctrl.paint_trail_dists if hasattr(paint_ctrl, 'paint_trail_dists') else None
                    dc = None
                    if driving_coords:
                        dc = [(pt[0], pt[1]) if not hasattr(pt, 'x')
                              else (pt.x, pt.y) for pt in driving_coords]
                    map_bgr = map_view.render(
                        vehicle_xy=veh_xy,
                        vehicle_yaw=veh_tf.rotation.yaw,
                        paint_trail=trail,
                        trail_dists=trail_dists,
                        driving_coords=dc)
                    if frame_count == 0 and map_bgr is not None:
                        print(f"  [V6 Map] first frame: {map_bgr.shape}, "
                              f"zoom={map_view.zoom:.2f}")
                except Exception as e:
                    if frame_count % 50 == 1:
                        print(f"  [V6 Map] render error: {e}")

            # --- V6: Separate display windows ---
            if overhead_img is not None:
                cv2.imshow("Overhead View", overhead_img)
            if depth_color is not None:
                cv2.imshow("Depth Camera", depth_color)
            if map_bgr is not None:
                cv2.imshow("Map View", map_bgr)
            cv_key = cv2.waitKey(1)
            if map_view is not None and cv_key != -1:
                map_view.handle_cv_key(cv_key)

            # --- V6: Publish 3 panels + map to RViz ---
            if rviz_pub is not None:
                try:
                    rviz_pub.publish_all(front_bgr, panel2_bgr, overhead_img,
                                         map_bgr, None)
                except Exception:
                    pass  # don't crash main loop on ROS errors

            # --- V6: Publish 3D markers to RViz ---
            if rviz_3d is not None:
                try:
                    veh_tf_ros = vehicle.get_transform()
                    if left_world and right_world:
                        rviz_3d.publish_edge_markers(left_world, right_world)
                    if driving_coords:
                        # Convert to (x, y) tuples for ROS Path msg
                        dc_xy = [(pt[0], pt[1]) if not hasattr(pt, 'x')
                                 else (pt.x, pt.y) for pt in driving_coords]
                        rviz_3d.publish_driving_path(dc_xy)
                    if poly_coeffs is not None:
                        rviz_3d.publish_poly_curve(poly_coeffs, veh_tf_ros)
                    if paint_ctrl.paint_trail:
                        nozzle_locs = [
                            carla.Location(x=p[0], y=p[1], z=veh_tf_ros.location.z)
                            for p in paint_ctrl.paint_trail if p is not None
                        ]
                        if nozzle_locs:
                            rviz_3d.publish_paint_trail(nozzle_locs)
                    rviz_3d.publish_vehicle_marker(veh_tf_ros)
                    status = (f"{perception.perception_mode} | "
                              f"Spd:{speed:.1f} | Edge:{edge_dist_r:.1f}m")
                    rviz_3d.publish_status_text(status, veh_tf_ros)
                except Exception as e:
                    if frame_count % 50 == 1:
                        print(f"  [V6 3D] Error: {e}")
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    elif map_view is not None:
                        map_view.handle_key(event.key)
        ...
        recorder.release()
        # V6: ROS shutdown
        if ROS_AVAILABLE and rviz_pub is not None:
            try:
                rospy.signal_shutdown('autostripe_v6 exit')
            except Exception:
                pass
        pygame.quit()
...
def _render_front_view(pg_screen, rgb_front, road_mask, scene,
    ...
    V6: Returns BGR composite for RViz Panel 1.
    """
    ...
    # V6: return BGR composite for RViz Panel 1
    return front_display


def _build_perception_detail(perception, depth_data, depth_color):
    """V6 Panel 2: mode-adaptive perception detail image.

    LUNA mode: SNE surface normal -> RGB visualization ((normal+1)/2 * 255)
    VLLiNet/GT mode: depth heatmap (COLORMAP_MAGMA)

    Returns BGR image (1248x384) or None.
    """
    if perception.perception_mode == PerceptionMode.LUNA:
        normal = perception.last_normal  # (3, H, W) float32
        if normal is not None:
            # normal range is roughly [-1, 1] -> map to [0, 255]
            vis = ((normal + 1.0) / 2.0 * 255.0).clip(0, 255)
            vis = vis.astype(np.uint8)           # (3, H, W)
            vis = vis.transpose(1, 2, 0)         # (H, W, 3) RGB
            return cv2.cvtColor(vis, cv2.COLOR_RGB2BGR)
    # VLLiNet / GT: use depth heatmap
    if depth_color is not None:
        return depth_color
    return None
